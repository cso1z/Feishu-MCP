...
export class FeishuApiService extends BaseApiService {
        ...
        try {
            ...
            do {
                ...
                const simplifiedBlocks = blocks.map((block: any) => {
                    ...
                    // 递归删除所有样式字段的函数
                    const removeStyleFields = (obj: any): any => {
                        if (obj && typeof obj === 'object') {
                            const result: any = {};
                            for (const [key, value] of Object.entries(obj)) {
                                // 跳过所有样式相关字段
                                if (key === 'text_element_style' || 
                                    key === 'style' || 
                                    key === 'align' || 
                                    key === 'folded' ||
                                    key === 'bold' ||
                                ...
                                if (value && typeof value === 'object') {
                                    result[key] = removeStyleFields(value);
                                } else {
                                    result[key] = value;
                                }
                            }
                    ...
                    // 应用样式字段删除
                    return removeStyleFields(simplified);
                });
     ...
     * @returns 更新结果
     */
    public async updateBlockTextContent(documentId: string, blockId: string, textElements: Array<{
        text?: string,
        equation?: string,
        style?: any
    }>): Promise<any> {
     ...
     * @returns 创建结果
     */
    public async createTextBlock(documentId: string, parentBlockId: string, textContents: Array<{
        text?: string,
        equation?: string,
        style?: any
    }>, align: number = 1, index: number = 0): Promise<any> {
    ...
    public async createImageBlock(
        ...
        try {
            ...
            } = await this.getImageBase64FromPathOrUrl(imagePathOrUrl);
